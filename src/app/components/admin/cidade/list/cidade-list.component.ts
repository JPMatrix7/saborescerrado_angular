...
export class CidadeListComponent implements OnInit {
  ...
  loadCidades(): void {
    this.cidadeService.findAll().subscribe({
      next: (data: Cidade[]) => this.cidades.set(data),
      error: (error: unknown) => console.error('Erro ao carregar cidades:', error)
    });
  }
  ...
  deleteCidade(id: number): void {
    if (confirm('Deseja realmente excluir esta cidade?')) {
      this.cidadeService.delete(id).subscribe({
        ...
        error: (error: unknown) => console.error('Erro ao excluir cidade:', error)
      });
    }
  }
